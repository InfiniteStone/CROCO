#include "cross_view_ray_casting_function_cpu.hpp"
        ...
        for (size_t idx = 0; idx < n ; ++idx)
        {
            ...
            if (h_top >= 0 && h_top < H && w_left >= 0 && w_left < W)
            {
                if (features_weight[n_v * H * W + h_top * W + w_left] > static_cast<scalar_t>(0.0))
                {
                    for (size_t i = 0; i < C; ++i)
                    {
                        grad_features[n_v * C * H * W + i * H * W + h_top * W + w_left] += \ 
                            grad_volume[n_v * n * C + idx * C + i] * conf_i \
                            / features_weight[n_v * H * W + h_top * W + w_left] \
                            * weight_left_top;
                        grad_features_weight[n_v * H * W + h_top * W + w_left] += \ 
                            -grad_volume[n_v * n * C + idx * C + i] * conf_i \
            ...
            if (h_down >= 0 && h_down < H && w_left >= 0 && w_left < W)
            {
                if (features_weight[n_v * H * W + h_down * W + w_left] > static_cast<scalar_t>(0.0))
                {
                    for (size_t i = 0; i < C; ++i)
                    {
                        grad_features[n_v * C * H * W + i * H * W + h_down * W + w_left] += \ 
                            grad_volume[n_v * n * C + idx * C + i] * conf_i \
                            / features_weight[n_v * H * W + h_down * W + w_left] \
                            * weight_left_down;
                        grad_features_weight[n_v * H * W + h_down * W + w_left] += \ 
                            -grad_volume[n_v * n * C + idx * C + i] * conf_i \
            ...
            if (h_top >= 0 && h_top < H && w_right >= 0 && w_right < W)
            {
                if (features_weight[n_v * H * W + h_top * W + w_right] > static_cast<scalar_t>(0.0))
                {
                    for (size_t i = 0; i < C; ++i)
                    {
                        grad_features[n_v * C * H * W + i * H * W + h_top * W + w_right] += \ 
                            grad_volume[n_v * n * C + idx * C + i] * conf_i \
                            / features_weight[n_v * H * W + h_top * W + w_right] \
                            * weight_right_top;
                        grad_features_weight[n_v * H * W + h_top * W + w_right] += \ 
                            -grad_volume[n_v * n * C + idx * C + i] * conf_i \
            ...
            if (h_down >= 0 && h_down < H && w_right >= 0 && w_right < W)
            {
                if (features_weight[n_v * H * W + h_down * W + w_right] > static_cast<scalar_t>(0.0))
                {
                    for (size_t i = 0; i < C; ++i)
                    {
                        grad_features[n_v * C * H * W + i * H * W + h_down * W + w_right] += \ 
                            grad_volume[n_v * n * C + idx * C + i] * conf_i \
                            / features_weight[n_v * H * W + h_down * W + w_right] \
                            * weight_right_down;
                        grad_features_weight[n_v * H * W + h_down * W + w_right] += \ 
                            -grad_volume[n_v * n * C + idx * C + i] * conf_i \
...
) 
{
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(features_weight.type(), "ray_casting_weight_calculation_forward_cpu_kernel", ([&] {
...
) 
{
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(grad_features_weight.type(), "ray_casting_weight_calculation_backward_cpu_kernel", ([&] {
...
) 
{
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(volume.type(), "ray_casting_sample_forward_cpu_kernel", ([&] {
...
) 
{
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(grad_volume.type(), "ray_casting_sample_backward_cpu_kernel", ([&] {
