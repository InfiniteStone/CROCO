#include "cross_view_ray_casting_function_cuda.cuh"
    ...
    for (size_t n_v = 0; n_v < n_view; ++n_v)
    {
        ...
        if (h_top >= 0 && h_top < H && w_left >= 0 && w_left < W)
        {
            if (features_weight[n_v * H * W + h_top * W + w_left] > static_cast<scalar_t>(0.0))
            {
                ...
                {
                    volume[n_v * n * C + idx * C + i] += features[n_v * C * H * W + i * H * W + h_top * W + w_left] \
        ...
        if (h_down >= 0 && h_down < H && w_left >= 0 && w_left < W)
        {
            if (features_weight[n_v * H * W + h_down * W + w_left] > static_cast<scalar_t>(0.0))
            {
                for (size_t i = 0; i < C; ++i)
                {
                    volume[n_v * n * C + idx * C + i] += features[n_v * C * H * W + i * H * W + h_down * W + w_left] \
        ...
        if (h_top >= 0 && h_top < H && w_right >= 0 && w_right < W)
        {
            if (features_weight[n_v * H * W + h_top * W + w_right] > static_cast<scalar_t>(0.0))
            {
                for (size_t i = 0; i < C; ++i)
                {
                    volume[n_v * n * C + idx * C + i] += features[n_v * C * H * W + i * H * W + h_top * W + w_right] \
        ...
        if (h_down >= 0 && h_down < H && w_right >= 0 && w_right < W)
        {
            if (features_weight[n_v * H * W + h_down * W + w_right] > static_cast<scalar_t>(0.0))
            {
                for (size_t i = 0; i < C; ++i)
                {
                    volume[n_v * n * C + idx * C + i] += features[n_v * C * H * W + i * H * W + h_down * W + w_right] \
    ...
    for (size_t n_v = 0; n_v < n_view; ++n_v)
    {
        ...
        if (h_top >= 0 && h_top < H && w_left >= 0 && w_left < W)
        {
            if (features_weight[n_v * H * W + h_top * W + w_left] > static_cast<scalar_t>(0.0))
            {
                for (size_t i = 0; i < C; ++i)
                {
                    atomicAdd(&grad_features[n_v * C * H * W + i * H * W + h_top * W + w_left], \ 
                        grad_volume[n_v * n * C + idx * C + i] * conf_i \
                        / features_weight[n_v * H * W + h_top * W + w_left] \
                        * weight_left_top);
                    atomicAdd(&grad_features_weight[n_v * H * W + h_top * W + w_left], \ 
                        -grad_volume[n_v * n * C + idx * C + i] * conf_i \
                        / (features_weight[n_v * H * W + h_top * W + w_left] * features_weight[n_v * H * W + h_top * W + w_left]) \
                        * weight_left_top);
                }
            }
        }
        ...
        if (h_down >= 0 && h_down < H && w_left >= 0 && w_left < W)
        {
            if (features_weight[n_v * H * W + h_down * W + w_left] > static_cast<scalar_t>(0.0))
            {
                for (size_t i = 0; i < C; ++i)
                {
                    atomicAdd(&grad_features[n_v * C * H * W + i * H * W + h_down * W + w_left], \ 
                        grad_volume[n_v * n * C + idx * C + i] * conf_i \
                        / features_weight[n_v * H * W + h_down * W + w_left] \
                        * weight_left_down);
                    atomicAdd(&grad_features_weight[n_v * H * W + h_down * W + w_left], \ 
                        -grad_volume[n_v * n * C + idx * C + i] * conf_i \
                        / (features_weight[n_v * H * W + h_down * W + w_left] * features_weight[n_v * H * W + h_down * W + w_left]) \
                        * weight_left_down);
                }
            }
        }
        ...
        if (h_top >= 0 && h_top < H && w_right >= 0 && w_right < W)
        {
            if (features_weight[n_v * H * W + h_top * W + w_right] > static_cast<scalar_t>(0.0))
            {
                for (size_t i = 0; i < C; ++i)
                {
                    atomicAdd(&grad_features[n_v * C * H * W + i * H * W + h_top * W + w_right], \ 
                        grad_volume[n_v * n * C + idx * C + i] * conf_i \
                        / features_weight[n_v * H * W + h_top * W + w_right] \
                        * weight_right_top);
                    atomicAdd(&grad_features_weight[n_v * H * W + h_top * W + w_right], \ 
                        -grad_volume[n_v * n * C + idx * C + i] * conf_i \
                        / (features_weight[n_v * H * W + h_top * W + w_right] * features_weight[n_v * H * W + h_top * W + w_right]) \
                        * weight_right_top);
                }
            }
        }
        ...
        if (h_down >= 0 && h_down < H && w_right >= 0 && w_right < W)
        {
            if (features_weight[n_v * H * W + h_down * W + w_right] > static_cast<scalar_t>(0.0))
            {
                for (size_t i = 0; i < C; ++i)
                {
                    atomicAdd(&grad_features[n_v * C * H * W + i * H * W + h_down * W + w_right], \ 
                        grad_volume[n_v * n * C + idx * C + i] * conf_i \
                        / features_weight[n_v * H * W + h_down * W + w_right] \
                        * weight_right_down);
                    atomicAdd(&grad_features_weight[n_v * H * W + h_down * W + w_right], \ 
                        -grad_volume[n_v * n * C + idx * C + i] * conf_i \
                        / (features_weight[n_v * H * W + h_down * W + w_right] * features_weight[n_v * H * W + h_down * W + w_right]) \
                        * weight_right_down);
                }
            }
        }
